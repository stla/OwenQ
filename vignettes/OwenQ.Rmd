...
Below we compare the values given by `ptOwen` and `pt` and the value given by Wolfram (returned by the command `N[CDF[NoncentralStudentTDistribution[4,70],80]]`):
...
The $H_1$ hypothesis is accepted at level $\alpha$ when this interval falls into the interval 
$[\Delta_1, \Delta_2]$.
...
The probability to get an inconclusive result can be obtained with the `OwenQ` package. We show and check that with the help of simulations below.
